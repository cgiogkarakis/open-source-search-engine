...
long RdbTree::addNode ( collnum_t collnum , 
	...
	if ( m_numUsedNodes == 0 ) {
		...
		m_numNegativeKeys = 0;
		m_numPositiveKeys = 0;
		m_numNegKeysPerColl[collnum] = 0;
		m_numPosKeysPerColl[collnum] = 0;
	}
...
long RdbTree::fastSaveBlock_r ( int fd , long start , long long offset ) {
	...
	// make sure errno did not get mangled by another clone process
	if ( br == offset - oldOffset && errno ) { 
	  return log("gb: clone errno bug");
	}
	// bitch on error
